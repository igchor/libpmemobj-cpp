...
struct test_case_1 : public test_case {
	void
	insert(nvobj::pool<root> &pop,
	       nvobj::persistent_ptr<persistent_map_type_string> &map) override
	{
...
struct test_case_2 : public test_case {
	void
	insert(nvobj::pool<root> &pop,
	       nvobj::persistent_ptr<persistent_map_type_string> &map) override
	{
		...
		/* two threads trying to insert the same element and one trying
		 * to erase
		 */
		parallel_xexec(
			3,
			[&](size_t thread_id,
				...
				if (thread_id == 0) {
					...
					UT_ASSERT(map->count(gen_key(
							  *map,
							  mt_insert_key)) == 1);

					pop.root()->reader_status =
						map->unsafe_erase(gen_key(
							*map, mt_insert_key));
				}
	      ...
	      nvobj::persistent_ptr<persistent_map_type_string> &map) override
	{
		...
		/* inserted node was removed by erase */
		UT_ASSERTeq(cleared_nodes_num, initial_nodes_num - 1);
...
test(int argc, char *argv[])
{
	...
	cases.emplace_back(new test_case_1);
	cases.emplace_back(new test_case_2);
