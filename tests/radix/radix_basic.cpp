...
void test_ref_stability(nvobj::pool<root> &pop)
{
	auto r = pop.root();
	...
	ref_type ab_ref = r->radix_str->emplace("ab", "ab").first->second;
	ref_type a_ref = r->radix_str->emplace("a", "a").first->second;
	ref_type acxxxy_ref = r->radix_str->emplace("acxxxy", "acxxxy").first->second;
	ref_type acxxxz_ref = r->radix_str->emplace("acxxxz", "acxxxz").first->second;

	UT_ASSERT(nvobj::string_view(ab_ref).compare("ab") == 0);
	UT_ASSERT(nvobj::string_view(a_ref).compare("a") == 0);
	UT_ASSERT(nvobj::string_view(acxxxy_ref).compare("acxxxy") == 0);
	UT_ASSERT(nvobj::string_view(acxxxz_ref).compare("acxxxz") == 0);
	}

	{
			using ref_type = typename container_int::reference;

	ref_type ab_ref = r->radix_int->emplace("ab", 1U).first->second;
	ref_type a_ref = r->radix_int->emplace("a", 2U).first->second;
	ref_type acxxxy_ref = r->radix_int->emplace("acxxxy", 3U).first->second;
	ref_type acxxxz_ref = r->radix_int->emplace("acxxxz", 4U).first->second;

	UT_ASSERT(ab_ref == 1U);
	UT_ASSERT(a_ref == 2U);
	UT_ASSERT(acxxxy_ref == 3U);
	UT_ASSERT(acxxxz_ref == 4U);
	}
...
test_erase(nvobj::pool<root> &pop)
{
	...
	std::unordered_map<std::string, typename container_string::iterator> its;
