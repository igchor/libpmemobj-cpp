		...
		return const_iterator(
			next_leaf<node::direction::Forward>(
				prev->template make_iterator<
					node::direction::Forward>(child_slot),
				prev),
			&root);
	}

	/* New key is a prefix of the leaf key. */
	if (diff == key.size()) {
		...
		return const_iterator(find_leaf<node::direction::Forward>(n),
				      &root);
	}

	/* Leaf key is a prefix of the new key. */
	if (diff == leaf_key.size()) {
